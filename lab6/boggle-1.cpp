...
int main() {
    ...
    std::map<std::string, int> player_hash;
    ...
    while (true) {
        ...
        std::cin >> input;
        if (input == "???") {
            auto_score = boggle.auto_find(player_hash, auto_words) - 3 * auto_words.size();
